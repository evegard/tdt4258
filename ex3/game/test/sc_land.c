    ...
    for (y = 0; y < GAME_HEIGHT; y++){
        for (x = 0; x < GAME_WIDTH; x++)
            if (game_player.x == x && game_player.y == y)
                printf(C_MILITARY "S");
            else if (y < GAME_TANK_SIZE && GAME_WIDTH - GAME_TANK_SIZE <= x)
                printf(C_MILITARY "T");
            else
                printf(game_is_scorched[y][x] ? C_SCORCHED "X" : C_GRASS "," );
        printf(C_END "\n");
    }
}
...
int main (){
    ...
    while ( TRUE ){
        ...
        if ( p_turn == 0 ) {
            ...
            while ( TRUE ) {
                ...
                switch ( s_move ){
                    ...
                    case GAME_MOVE_OK:
                        if ( s_dir[0] != 0 )
                            printf ( C_ERROR "ERROR: " C_ERROR_NOBOLD "%c is an illegal character input!" C_END "\n", s_dir[0] );
                        else
                            printf ( "It is the soldier's turn:\n" );
                    break;

                    case GAME_MOVE_IS_SCORCHED:
                        printf (C_ERROR "ERROR: " C_ERROR_NOBOLD "The tile you're trying to move to is scorched!" C_END "\n");
                        break;

                    case GAME_MOVE_OOB:
                        printf (C_ERROR "ERROR: " C_ERROR_NOBOLD "The tile you're trying to is out of bounds!" C_END "\n");
                        break;
                }
                printf ( "Soldier - choose a direction to go to: w, a, s, d\n" );
                scanf ( "%s", &s_dir[0] );
                switch (s_dir[0]) {
        ...
        if ( p_turn == 1) {
            ...
            while (TRUE){
                print_map();
                if ( game_position_equals(shot_pos, GAME_SHOT_ILLEGAL_ARGUMENT) )
                    printf ( C_ERROR "ERROR: " C_ERROR_NOBOLD "The arguments you've given is out of the legal range!" C_END "\n" );
                else
                    printf ( "It is time for the tank to shoot.\n" );
                printf ( "Tanks, choose the direction (0-%d) and strength (0-%d) on the shot:\n",
                ...
                else if (!game_position_equals(shot_pos, GAME_SHOT_ILLEGAL_ARGUMENT)){
                    print_map();
                    if (shot_pos.x < 0 && shot_pos.y < 0){
                        printf ( "The tanks has destroyed all chances for the\n"
                                 "soldier to reach the tank! (Hit at %d, %d) \n",
